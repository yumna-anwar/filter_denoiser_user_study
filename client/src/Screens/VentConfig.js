...
const VentConfig = () => {
  ...
  const fetchfilter = async () => {
    try {
      const response = await CommonService.GetFilterB();
      if (response.success) {
  ...
  if (action === 'applyPlay') {
    ...
    try {
      ...
      const response = await CommonService.RunFilterBtest(payload);
      setgainvaluesString(mhagainparam)
      if (response.success) {
    ...
    var response = await CommonService.AddFilterB(payload);
    if (response.success) {
      toast.success(response.message);
    } else {
      toast.error(response.message);
...
const handlePlayPauseAudio = () => {
  ...
  const handleReplayAudio = () => {
    ...
    if (audioElement) {
      ...
      const audioSourceUrl = "http://localhost:3001/assets/test_sentence/filterB-test/stereo_ISTS.wav";
  ...
  const handleFilterSelection = async (e) => {
    ...
    const response = await CommonService.GetFilterBById(newFilterId);
    const data = response.data
    ...
    <div className={styles.centeringContainer}>
      <form onSubmit={handleSubmit} className={styles.formLayout}>
        ...
        <button
          ...
          <audio ref={audioRef} >
            <source src="http://localhost:3001/assets/test_sentence/filterB-test/stereo_ISTS.wav" type="audio/wav" />
